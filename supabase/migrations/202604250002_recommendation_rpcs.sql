...
create trigger student_forms_touch_updated_at
...
begin
  select university_name into v_home_university
  from public.home_university_maps
  where district_group = p_home_district_group;
  ...
  low_rows as (
    ...
    where cap_round_1 < p_rank
    order by cap_round_1 desc
    limit 40
  ),
  high_rows as (
    select * from best_per_branch
